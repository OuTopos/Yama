...
function entities_fplayer.new(x, y, z, viewport)
	local self = {}
	...
	local tilesetOversized = "tilesets/lpcfemaledark"
	...
	function self.updateInput(dt)
		...
		if state == "stand" or state == "walk" then
			...
			if love.keyboard.isDown("lctrl") or love.joystick.isDown(2, 1) then
				state = "sword"
			...
			elseif yama.g.getDistance(0, 0, love.joystick.getAxis(2, 1), love.joystick.getAxis(2, 2)) > 0.2 then
				state = "walk"
				nx = love.joystick.getAxis(2, 1)
				ny = love.joystick.getAxis(2, 2)
				direction = math.atan2(ny, nx)
				aim = direction
				vmultiplier = yama.g.getDistance(0, 0, love.joystick.getAxis(2, 1), love.joystick.getAxis(2, 2))
		...
		if state == "walk" then
			if love.keyboard.isDown("lshift") or love.joystick.isDown(2, 5) then
				vmultiplier = vmultiplier * 3
			end
			fx = sx * velocity * vmultiplier * math.cos(direction)
			fy = sx * velocity * vmultiplier * math.sin(direction)
		...
		if yama.g.getDistance(0, 0, love.joystick.getAxis(2, 4), love.joystick.getAxis(2, 5)) > 0.2 then
			local nx = love.joystick.getAxis(2, 4)
			local ny = love.joystick.getAxis(2, 5)
