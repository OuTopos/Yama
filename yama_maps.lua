...
function maps.new(vp)
	...
	function public.load(vp, path, spawn)
		private.swarm = vp.getSwarm()
		...
		if maps.data[path] then
			...
			if public.data.orientation == "orthogonal" then
				...
				vp.getCamera().setBoundaries(0, 0, public.data.width * public.data.tilewidth, public.data.height * public.data.tileheight)
				...
				if public.data.spawns[spawn] then
					public.data.player = yama.entities.new(public.data.properties.player_entity, public.data.spawns[spawn].x + public.data.spawns[spawn].width / 2, public.data.spawns[spawn].y + public.data.spawns[spawn].height / 2, public.data.spawns[spawn].properties.z or 0, vp)
				else
					public.data.player = yama.entities.new(public.data.properties.player_entity, 200, 200, 0, vp)
				end
				vp.getSwarm().insert(public.data.player)
		...
		--vp.setScale(public.data.properties.sx, public.data.properties.sy, false)

		-- Set physics world
		physics.setWorld(private.swarm.getWorld())
		...
		vp.getCamera().follow = public.data.player
	...
	function public.shape(object)
		if object.shape == "rectangle" then
			...
			if object.gid then
				--Tile
				local body = love.physics.newBody(private.swarm.getWorld(), object.x, object.y-public.data.tileheight, "static")
				...
				--Rectangle
				local body = love.physics.newBody(private.swarm.getWorld(), object.x, object.y, "static")
				local shape = love.physics.newRectangleShape(object.width/2, object.height/2, object.width, object.height)
				return love.physics.newFixture(body, shape)
			end
		elseif object.shape == "ellipse" then
			--Ellipse
			local body = love.physics.newBody(private.swarm.getWorld(), object.x+object.width/2, object.y+object.height/2, "static")
